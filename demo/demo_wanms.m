...
Z = rastrigin([X(:)'; Y(:)']);
Z = reshape(Z,size(X));
surf(X,Y,Z,'linestyle','none','FaceAlpha',1.0);
colormap("parula")
...
%% Loop for selecting best solution
figure
...
clc; clf;
[M, c] = contour(XX, YY, ZZ, 'ShowText', 'off', 'HandleVisibility', 'off');
...
maksIter = 100;
for iter=1:maksIter
    [Simplex,F,clrCode] = wAdaptiveNelderMead(Cost,Simplex,F);
    X = Simplex(:,1); Y = Simplex(:,2);
    if (iter<=5) || (mod(iter,10)==0)
        fprintf('%2d (%5.3e, %5.3e) (%5.3e, %5.3e) (%5.3e, %5.3e) %6.3e %s\n', iter, Simplex(1,1), Simplex(1,2), ...
            Simplex(2,1), Simplex(2,2), Simplex(3,1), Simplex(3,2), F(1), op);
        fill(X,Y,clrCode,'FaceAlpha',0.3,'LineWidth',1.25,'HandleVisibility', 'off');
        savefig(['figs\wanms\rastrigin_wanms_' num2str(iter) '.fig']);
        print(gcf,['figs\wanms\rastrigin_wanms_' num2str(iter)],'-depsc','-r300')
        print(gcf,['figs\wanms\rastrigin_wanms_' num2str(iter)],'-dpng','-r300')
    end
end
disp('-----------------------------------------------------------------------------------')
plot(Simplex(1,1),Simplex(1,2),'bs','MarkerSize',12,'HandleVisibility', 'on','DisplayName','wANMS Solution','MarkerFaceColor','green')
plot(0,0,'m.','markersize',24,'HandleVisibility', 'off');
savefig(['figs\wanms\rastrigin_wanms_' num2str(maksIter) '.fig']);
print(gcf,['figs\wanms\rastrigin_wanms_' num2str(maksIter)],'-depsc','-r300')
print(gcf,['figs\wanms\rastrigin_wanms_' num2str(maksIter)],'-dpng','-r300')
hold off
